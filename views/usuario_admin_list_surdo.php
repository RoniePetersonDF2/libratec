...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($operacao === 'deletar') {
        ...
        if ($stmt->rowCount()) {
            header('location: usuario_admin_list_surdo.php?success=Usuário excluído com sucesso!');
        } else {
            header('location: usuario_admin_list_surdo.php?error=Erro ao excluir usuário!');
        }
    }
}
# cria uma consulta banco de dados buscando todos os dados da tabela usuarios 
# ordenando pelo campo perfil e nome.
$query = "SELECT * FROM `libratecdb`.`surdo` order by perfil, nome_surdo ";
$stmt = $dbh->prepare($query);
        ...
        <div class="main_opc">
            ...
            <div class="main_stage">
                <div class="main_stage_content">
                    <div>
                        <button class="btn" style="min-height: 40px; margin-bottom: 10px;" onclick="javascript:window.location='usuario_admin_add_surdo.php'">Novo usuário</button>
                    </div>
                    <article>
                        <header>
                            ...
                            <table border="0" width="1300px" class="table">
                                ...
                                if ($rows) {
                                    ...
                                    foreach ($rows as $row) { ?>
                                        <tr>
                                            <td><?= $count ?></td>
                                            <td><?= $row['nome_surdo'] ?></td>
                                            <td><?= $row['email_surdo'] ?></td>
                                            <td><?= $row['perfil'] ?></td>
                                            <td>
                                                <div style="display:flex;">
                                                    <a href="usuario_admin_upd_surdo.php?id=<?= $row['id_surdo'] ?>" class="btn">Editar</a>&nbsp;
                                                    <form action="" method="post">
                                                        <input type="hidden" name="id" value="<?= $row['id_surdo'] ?>" />
