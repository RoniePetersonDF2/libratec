...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($operacao === 'deletar') {
        ...
        if ($stmt->rowCount()) {
            header('location: usuario_admin_list_empresa.php?success=Usuário excluído com sucesso!');
        } else {
            header('location: usuario_admin_list_empresa.php?error=Erro ao excluir usuário!');
        }
    }
}
# cria uma consulta banco de dados buscando todos os dados da tabela usuarios 
# ordenando pelo campo perfil e nome.
$query = "SELECT * FROM `libratecdb`.`empresa` order by perfil, nome_empresa ";
$stmt = $dbh->prepare($query);
        ...
        <div class="main_opc">
            ...
            <div class="main_stage">
                <div class="main_stage_content">
                    <div>
                        <button class="btn" style="min-height: 40px; margin-bottom: 10px;" onclick="javascript:window.location='usuario_admin_add_empresa.php'">Novo usuário</button>
                    </div>
                    <article>
                        <header>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div style="display:flex;">
                                                    <a href="usuario_admin_upd_empresa.php?id=<?= $row['id_empresa'] ?>" class="btn">Editar</a>&nbsp;
